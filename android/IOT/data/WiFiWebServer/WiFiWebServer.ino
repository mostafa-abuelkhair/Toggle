...
int r[]={1,1,1,1,1,1,1,1},sr[]={1,1,1,1,1,1,1,1},tr[]={1,1,1,1,1,1,1,1},kr[]={0,0,0,0,0,0,0,0},hr[]={1,1,1,1,1,1,1,1};
int ser=0,tn=1,tx=800,sn=1,sx=800,kn=1,kx=100,hn=1,hx=100;
int smoker = 0;  
int knockr = 0;  
...
delay(10);
 ...
 pinMode(D0, OUTPUT); pinMode(D1, OUTPUT);pinMode(D2, OUTPUT);pinMode(D3, OUTPUT);pinMode(D4, OUTPUT);pinMode(D5, OUTPUT);pinMode(D6, OUTPUT);pinMode(D8, OUTPUT);pinMode(10, OUTPUT);//;pinMode(D9, OUTPUT);pinMode(D10, OUTPUT);
...
while (Serial.available() == 0) {
 ...
 rs= Serial.readString();
 rs.toCharArray(ssid, rs.length()+1);
 ...
 while (Serial.available() == 0) {
  delay(100);
   ...
   rs= Serial.readString();
 rs.toCharArray(password, rs.length()+1);
...
if (mx==300) {mx=0;
 ...
 if (hum>=hx){
      ser=1;digitalWrite(serin,ser);writerelays(hr);
     }}
...
WiFiClient client = server.available();
 ...
 /* // Match the request
  ...
  y=req.substring(5,(req.indexOf("HTTP")-1));
  relays(y);
 ...
 /*  else if (y=="smoke"){
    ...
    client.print(p2(sr,sn,sx,"smoke",smoker,"Smoke Sensor Setting"));
       ...
       hum = dht.readHumidity();
          client.flush();
    client.print(p2(hr,hn,hx,"hum",hum,"Humidity Sensor Setting"));
     }

  else if (y=="knock"){
       knockr = analogRead(analogsensor);
          client.flush();
    client.print(p2(kr,kn,kx,"knock",knockr,"Vibration Sensor Setting"));
     }
     
   else if (y.indexOf("set") != -1){
    String z[]={"mtj","kk","kk","kl","mtj","kk","kk","kl","mtj","kk","kk","kl"};
  ...
  if (z[1]=="hum"){
    for(int i=0;i<=7;i++){hr[i]=z[i+4].toInt();}
    hx=z[2].toInt();hn=z[3].toInt();
    client.flush();
    client.print(p1());
    }
...
<div style=\"background:#ffff66;font-size: 20px;text-align: center;margin: auto;margin-top: 10px;cursor: pointer; \" onclick=\"location.replace('http://"+ip.toString()+"/hum');\">Humidity Sensor Setting</div>\
...
<div style=\"width :320px;margin: auto;margin-top: 15px;font-size: 30px;\"><span style=\"display: inline;\">Sensor</span><div class=\"swhd\" onclick=\"sn=sn==1? 0:1;relay();\"><span class=\"son\" id=\"n9\">ON</span><span class=\"soff\" id=\"f9\">OFF</span></div></div>\
...
<script>\
 ...
 for(i=1;i<=9;i++){\
 var on =document.getElementById('n'+i), off =document.getElementById('f'+i);if (i!=9){var nc =document.getElementById('c'+i);}\
 if(rx[(i-1)]==0){ on.style.visibility='hidden' ;if (i!=9){nc.style.visibility='hidden';} off.style.visibility='visible';}else if(rx[(i-1)]==1){off.style.visibility='hidden' ;if (i!=9){nc.style.visibility='hidden';} on.style.visibility='visible';}else if(rx[(i-1)]==2){off.style.visibility='hidden' ;nc.style.visibility='visible' ;on.style.visibility='hidden';}\
 }\
 };\
 relay();\
...
void writerelays(int w[]){
  if (w[0]!=2){digitalWrite(re1,w[0]);r[0]=w[0];}
  if (w[1]!=2){digitalWrite(re2,w[1]);r[1]=w[1];}
  if (w[2]!=2){digitalWrite(re3,w[2]);r[2]=w[2];}
  if (w[3]!=2){digitalWrite(re4,w[3]);r[3]=w[3];}
  if (w[4]!=2){digitalWrite(re5,w[4]);r[4]=w[4];}
  if (w[5]!=2){digitalWrite(re6,w[5]);r[5]=w[5];}
  if (w[6]!=2){digitalWrite(re7,w[6]);r[6]=w[6];}
  if (w[7]!=2){digitalWrite(re8,w[7]);r[7]=w[7];}
}
